...
public class MCE_UnmintCommand {
    public MCE_UnmintCommand(CommandDispatcher<CommandSourceStack> dispatcher) {
        dispatcher.register(Commands.literal("unmint").executes(this::executeArgumentless).then(Commands.argument("amount", IntegerArgumentType.integer(1, 64)).executes(this::execute)));
    }

    private int executeArgumentless(CommandContext<CommandSourceStack> context) {
        ServerPlayer player = context.getSource().getPlayer();
        Gson gson = new Gson();
        JsonObject request = gson.fromJson(MCERequests.UNMINT.requestTemplate, JsonObject.class);
        ItemStack playerMainHand = player.getMainHandItem();
        int count = playerMainHand.getCount();
        if (!(playerMainHand.getItem().asItem() == MCE_Items.RESOURCE_TOKEN.asItem())) {
            context.getSource().sendFailure(Component.literal("Not an Unmintable Item"));
            return 0;
        }
        if (!playerMainHand.isEmpty() && count <= playerMainHand.getCount()) {
            request.getAsJsonObject("dat").addProperty("amount", count);
            request.getAsJsonObject("dat").addProperty("resource_id", playerMainHand.get(MCEDataComponentTypes.CURRENCY_ID));
            ...
            if (response.get("response").getAsString().equals("accept") && response.get("error").getAsString().equals("None")) {
                player.addItem(new ItemStack(
                        Resource_Token.getUnmintedItem(playerMainHand), count));
            ...
            context.getSource().sendFailure(Component.literal("Insufficent Items for Transaction"));
            return 0;
        }
    }
